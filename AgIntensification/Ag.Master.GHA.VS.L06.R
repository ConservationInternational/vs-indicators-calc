...
eval(run1)
  # only keep relevant LANDSCAPE and YEAR: 
    #LANDSCAPE: L06
    #YEAR: 2014
    field_data <- subset(FieldData.GHA.VS, Landscape == "L06" & Data.entry.year == "2014")
    num_crops <- subset(NumCrops.GHA.VS, Landscape == "L06" & Data.entry.year == "2014", 
                          select = c(country_y3hhid_plotnum, y3_hhid, plotnum, numcrops))
    pct_maize <- subset(PctMaize.GHA.VS, Landscape == "L06" & Data.entry.year == "2014", 
...
setwd(data_dir1)
run2 <- parse("Ag.PotentialYield.GHA.R") 
eval(run2)
  AI11 <- yp.L06
  
# AI20 - cultivated_area in ha
  # source: "GHA.AreaCultivated.Family5.csv"output table 
  # generated from "Ag.CultivatedAreaFamily5.GHA.R" script  
setwd(out_dir1)
Family5 <- read.csv("GHA.AreaCultivated.Family5.csv")
...
write.csv(output, file = "Ag.Intensification.GHA.VS.L06.2014.csv")