...
eval(run1)
  # only keep relevant LANDSCAPE and YEAR: 
    #LANDSCAPE: L04
    #YEAR: 2014
    field_data <- subset(FieldData.GHA.VS, Landscape == "L04" & Data.entry.year == "2014")
    num_crops <- subset(NumCrops.GHA.VS, Landscape == "L04" & Data.entry.year == "2014", 
                          select = c(country_y3hhid_plotnum, y3_hhid, plotnum, numcrops))
    pct_maize <- subset(PctMaize.GHA.VS, Landscape == "L04" & Data.entry.year == "2014", 
...
write.csv(output, file = "Ag.Intensification.GHA.VS.L04.2014.csv")