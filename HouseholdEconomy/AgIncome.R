...
hhe <- hhe %>% group_by(hh_refno, round) %>% summarize(income_own=sum(income_own, na.rm=T),
...
inc1 <- tbl(con, 'c__agric_crops_by_hh') %>%
  select(hh_refno, round,
         ag5a_03, ag5a_26) %>%
  data.frame
...
inc1 <- inc1 %>% group_by(hh_refno, round) %>% summarize(income_crops=sum(income_crops, na.rm=T))
...
inc2 <- tbl(con, 'c__agric_perm_crop') %>%
  select(hh_refno, round, ag7a_04) %>%
  data.frame
...
inc3 <- tbl(con, 'c__agric_livestock_byproduct') %>%
  select(hh_refno, round, ag10b_06) %>%
  data.frame
...
inc4 <- tbl(con, 'c__agric_byprod') %>%
  select(hh_refno, round, ag09_08, ag09_11) %>%
  data.frame
...
inc4 <- inc4 %>% group_by(hh_refno, round) %>% summarize(income_byprod=sum(income_byprod, na.rm=T))
...
inc5 <- tbl(con, 'c__agric_livestock') %>%
  select(hh_refno, round, ag10a_21, ag10a_27, ag10a_34) %>%
  data.frame
...
inc5 <- inc5 %>% group_by(hh_refno, round) %>% summarize(income_lvstk = sum(income_lvstk, na.rm=T))
...
inc6 <- tbl(con, 'c__agric_crops_by_field') %>%
  select(hh_refno, round, ag4a_21) %>%
  data.frame

inc6 <- inc6 %>% group_by(hh_refno, round) %>% summarize(cost_seeds = sum(ag4a_21, na.rm=T))
...
inc7 <- tbl(con, 'c__agric_field_details') %>%
  select(hh_refno, round, ag3a_32, ag3a_61, ag3a_22) %>%
  data.frame

inc7 <- inc7 %>% group_by(hh_refno, round) %>% summarize(cost_org_fert = sum(ag3a_22, na.rm=T), 
...
allvars <- allvars %>% select(country, landscape_no, hh_refno, round,
...
income <- allvars %>% group_by(country, landscape_no) %>%
