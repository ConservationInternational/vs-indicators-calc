###############################################################################
# Calculates mean precipitation for Tanzania for Vital Signs GRP presentation
###############################################################################
...
# What periods (in months) should anomalies should be calculated for?
anom_periods <- c(12, 24)
...
# Function to calculate running total precipitation for a given period
...
# Function to calculate anomaly for a given period
calc_anom_n_mth <- function(p, n_mth, ...) {
    ...
    period_mean <- foreach(n=1:12, .combine=abind, .export='runtot') %do% {
        out <- apply(runtot[ , , seq(from=n, by=12, length.out=round(dim(p)[3]/12))], c(1, 2), mean, na.rm=TRUE)
...
# Calculate running totals and anomalies for 12 and 24 month periods
for (anom_period in anom_periods) {
    timestamp()
    print(paste('Calculating', anom_period, 'running total...'))
    ...
    print(paste('Calculating', anom_period, 'month anomaly...'))
...
#
# plot(anom[[45]], zlim=c(-1200,1200))
# plot(anom[[46]], zlim=c(-1200,1200))
# plot(anom[[47]], zlim=c(-1200,1200))
# plot(anom[[63]], zlim=c(-1200,1200))
# plot(anom[[64]], zlim=c(-1200,1200))
# plot(anom[[65]], zlim=c(-1200,1200))
#
# plot(runtotal[[45]])
# plot(runtotal[[46]])
# plot(runtotal[[47]])
