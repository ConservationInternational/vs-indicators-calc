...
fs <- tbl(vs_db, 'c__household_secI') %>% data.frame
...
fs <- fs %>% select(country, landscape_no, hh_refno, round, shortage_year, CSI, months_insecurity, number_meals=hh_i031, hfias)

diet <- tbl(vs_db, 'c__household_secK2') %>% data.frame
...
diet <- diet %>% select(country, landscape_no, hh_refno, round, diversity)

##Nonfood spending
##Do we need Sec L?

nonfood_spending <- tbl(vs_db, "c__household_secL") %>% data.frame
nonfood_spending_b <- tbl(vs_db, "c__household_secM") %>% data.frame

# keep the first 4 columns, while subsequently dropping every other column
df <- nonfood_spending[ , c('country', 'landscape_no', 'hh_refno', 'round', names(nonfood_spending)[grepl('_2$', names(nonfood_spending))])]

# melt to long shape
df <- melt(df, 
           id.vars = c("country", "landscape_no", "hh_refno", "round"),
...
nfs <- df %>% group_by(country, landscape_no, hh_refno, round) %>%
  summarize(total.spent.a=sum(amount.spent, na.rm=T))


df2 <- nonfood_spending_b[ , c('country', 'landscape_no', 'hh_refno', 'round', names(nonfood_spending_b)[grepl('_2$', names(nonfood_spending_b))])]
...
nfs2 <- df2 %>% group_by(country, landscape_no, hh_refno, round) %>%
...
nfs3 <- nfs3 %>% select(country, landscape_no, hh_refno, round, Nonfood.Spending)


##Food Spending
df <- tbl(vs_db, "c__household_secK1") %>% data.frame #FS 6

food <- df %>% group_by(country, landscape_no, hh_refno, round) %>% 
