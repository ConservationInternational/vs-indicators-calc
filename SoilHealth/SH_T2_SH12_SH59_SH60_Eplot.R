################################################################################
#SOIL HEALTH THREAD
#Tier 2a/4 staging of SH59 (soil fertility) and SH60 (soil C deficit) for eplots
...
#Load data frame with soil predicted values from lab and landcover data merged with GPS coordinate information
run1<-ci.repo$parseRCode("SoilHealth/SH_T4_MergingEplotGPSwithLabdata.R")
eval(run1)
rm(list=ls()[! ls() %in% c("Tier4SoilPredicted","Tier4landcover","dir_inputscripts","dir_output", "Mainsub")])

#Note that current column names do not conform to data dictionary
names(Tier4SoilPredicted)
...
#CLEANING STEP - Renaming variables
...
#Select required variables and required depth
Tier4SoilPredicted_key <- subset(Tier4SoilPredicted, sample_depth_bottom==20, select=c(Eplot, Landscape, Country, Eplot.Code,
  sample_depth_bottom,sample_depth_top,Acidified.Carbon, Clay_CNLS,Clay_icraf, ExAcidity,
  Exch.Al, m3.Ca, m3.K, m3.Mg, m3.P, pH, m3.S, Sand_CNLS, Sand_icraf, Silt_CNLS, Silt_icraf,
  GPS.SE.accuracy, GPS.SE.latitude, GPS.SE.longitude))
  
#Assigning measurement codes
attach(Tier4SoilPredicted_key)
Tier4SoilPredicted_key$SH6d <- m3.P
Tier4SoilPredicted_key$SH6e <- m3.S
Tier4SoilPredicted_key$SH8 <- pH
Tier4SoilPredicted_key$SH10 <- Exch.Al
Tier4SoilPredicted_key$SH12 <- Acidified.Carbon
Tier4SoilPredicted_key$SH14a <- Clay_icraf
Tier4SoilPredicted_key$SH14b <- Silt_icraf
...
#MODELS
###########################################
#SH41 - Critical threshold soil percent aluminum saturation (SH10 / SH6a  + SH6b + SH6c + SH10)  x 100 (Sanchez et al. 2003)
...
Tier4SoilPredicted_key$SHFI6 [Tier4SoilPredicted_key$SH8 <= 5.5 & Tier4SoilPredicted_key$SHFI6==888 ] <- Tier4SoilPredicted_key$SHFI6_B [Tier4SoilPredicted_key$SH8 <= 5.5 & Tier4SoilPredicted_key$SHFI6==888 ]
...
summary(Tier4SoilPredicted_key$SH59)
...
SH_Tier2_4_processed <- subset(Tier4SoilPredicted_key, sample_depth_bottom==20, select=c(Country, Landscape, Eplot, Eplot.Code,sample_depth_bottom,sample_depth_top,
...
SH_Tier2_4_SH12_SH59_SH60 <- subset(Tier4SoilPredicted_key, sample_depth_bottom==20, select=c(Country, Landscape, Eplot, Eplot.Code,
    sample_depth_bottom,sample_depth_top, SH12, SH59, SH60, GPS.SE.accuracy, GPS.SE.latitude, 
    GPS.SE.longitude))

#Write output according to indicator deliverable
write.csv(SH_Tier2_4_SH12_SH59_SH60,"SH_Tier2_4_SH12_SH59_SH60_Eplot.csv")
s3 <- newS3()
s3$writeS3(bucket = "ci-vsindicators", source_path = "SH_Tier2_4_SH12_SH59_SH60_Eplot.csv", target_path = "Soil_Health/SH_Tier2_4_SH12_SH59_SH60_Eplot.csv")
