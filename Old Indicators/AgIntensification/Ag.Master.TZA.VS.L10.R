...
eval(run1)
  # only keep relevant LANDSCAPE and YEAR: 
    #LANDSCAPE: L10
    #YEAR: 2014
    field_data <- subset(FieldData.TZA.VS, Landscape == "L10" & Data.entry.year == "2014")
    num_crops <- subset(NumCrops.TZA.VS, Landscape == "L10" & Data.entry.year == "2014", 
                          select = c(country_y3hhid_plotnum, y3_hhid, plotnum, numcrops))
    pct_maize <- subset(PctMaize.TZA.VS, Landscape == "L10" & Data.entry.year == "2014", 
...
write.csv(output, file = "Ag.Intensification.TZA.VS.L10.2014.csv")