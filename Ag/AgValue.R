...
agvalue <- tbl(con, 'flagging__agric_crops_by_field') %>%
  select(`Household ID`, Round, ag4a_16, ag4a_21) %>%
  data.frame

agvalue <- agvalue %>% group_by(Household.ID, Round) %>% summarize(Crops = sum(ag4a_16, na.rm=T) - sum(ag4a_21, na.rm=T))
...
ag1 <- ag1 %>% group_by(Household.ID, Round) %>% summarize(Inputs = sum(ag3a_22, na.rm=T) + sum(ag3a_61, na.rm=T) + sum(ag3a_22, na.rm=T))
...
ag2 <- tbl(con, 'flagging__agric_perm_crops_by_field') %>%
  select(`Household ID`, Round, ag6a_vs_09) %>%
  data.frame

ag2 <- ag2 %>% group_by(Household.ID, Round) %>% summarize(Permanent.Crops = sum(ag6a_vs_09, na.rm=T))
...
ag3 <- tbl(con, 'flagging__agric_livestock_byproduct') %>%
  select(`Household ID`, Round, ag10b_1b) %>%
  data.frame

ag3 <- ag3 %>% group_by(Household.ID, Round) %>% summarize(Livestock.Byproducts = sum(ag10b_1b, na.rm=T))
...
ag4 <- tbl(con, 'flagging__agric_livestock') %>%
  select(Country, `Landscape #`, `Household ID`, `Animal name`, Round, ag10a_25, ag10a_26, ag10a_27, ag10a_20, ag10a_21, ag10a_34) %>%
  data.frame
...
ag4 <- ag4 %>% group_by(Household.ID, Round) %>% summarize(Livestock=(sum(value)-sum(ag10a_34, na.rm=T)))
...
ag5 <- tbl(con, 'flagging__agric_byprod') %>%
  select(Country, `Landscape #`, `Household ID`, Round, ag09_11, ag09_04_1, ag09_05, ag09_06_1, ag09_08, `Crop ID`) %>%
  data.frame
...
ag5 <- ag5 %>% group_by(Household.ID, Round) %>% summarize(Crop.Byproducts=(sum(value)-sum(ag09_11, na.rm=T)))
